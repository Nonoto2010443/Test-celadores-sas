...
agent_communication:
  - agent: "main"
    message: "User reported two formatting issues: 1) Questions starting with lowercase after '¿' (e.g., '¿cual de las siguientes...'), 2) Purple text appearing where it shouldn't (only topic banner should be purple). FIXES IMPLEMENTED: 1) Created and executed fix_capitalization_after_question_mark.py script that corrected 850 questions by capitalizing first letter after '¿' while preserving 'art.' abbreviation. 2) Updated ExamPage.css with explicit !important rules to prevent purple color inheritance on question text and options - only .question-number and .question-tema should remain purple. Ready for backend and frontend testing to verify both fixes work correctly."
  - agent: "testing"
    message: "❌ BACKEND TESTING COMPLETE - CRITICAL CAPITALIZATION ISSUE IDENTIFIED: Database fix script worked correctly (0 questions needed fixing in stored data), but AI question generation system still produces capitalization violations. Found 3 violations in generated exam: questions starting with '¿c' instead of '¿C'. Root cause: AI generation prompts in server.py need to include capitalization rules for Spanish question marks. Database integrity confirmed (16,510 questions). The issue is NOT in stored questions but in real-time AI generation. Main agent must update AI prompts to enforce capitalization after '¿' in generate_questions_with_ai() function."
  - agent: "main"
    message: "AI GENERATION FIXED: Updated AI prompts in both server.py (generate_questions_with_ai) and generate_ai_questions_batch.py to enforce Spanish capitalization rules after '¿'. Added explicit rule with examples to system message: 'CAPITALIZACIÓN ESPAÑOLA DESPUÉS DE ¿: La palabra que sigue inmediatamente al signo de interrogación de apertura ¿ debe ir en mayúscula'. Backend restarted. Ready for comprehensive backend testing to verify: 1) Database questions remain correctly capitalized, 2) AI-generated questions now follow capitalization rules, 3) CSS changes prevent purple text in non-intended areas."
  - agent: "testing"
  ...
  - agent: "testing"
    message: "✅ CRITICAL TIMEOUT FIX TESTING COMPLETE - The timeout issue has been successfully resolved! Comprehensive testing of the async justifications implementation shows: 1) EXAM SUBMISSION: Now completes in 0.04 seconds (125x faster than 5s requirement), 2) IMMEDIATE RESULTS: Users get instant feedback with scores without waiting for AI processing, 3) ASYNC JUSTIFICATIONS: New endpoint POST /api/results/{result_id}/generate-justification works perfectly for on-demand justification generation, 4) CACHING: Justifications are properly cached and retrieved efficiently, 5) QUALITY: Generated justifications include proper source citations and are concise as required. The critical timeout bug that was blocking user experience has been completely fixed. The system is now production-ready for exam submissions and async justification loading."
  - agent: "testing"
    message: "❌ CAPITALIZATION FIX RE-TEST FAILED - Comprehensive testing of capitalization fix reveals the issue persists. Tested 3 exams (150 questions total) and found 6 capitalization violations across 9 AI-generated questions. ROOT CAUSE IDENTIFIED: The problem is NOT in the updated AI prompts (which are correct), but in the pre-generated AI questions stored in the 'preguntas_ia' database collection. These stored questions still contain violations like '¿c' should be '¿C' and '¿q' should be '¿Q'. Database questions (141/150) are perfectly clean. CRITICAL FINDING: The system uses pre-generated AI questions from the database rather than generating them in real-time, so updating the prompts alone doesn't fix existing stored questions. SOLUTION NEEDED: Either regenerate the entire preguntas_ia collection with the updated prompts OR create a database cleanup script to fix capitalization in existing AI questions."
  - agent: "main"
